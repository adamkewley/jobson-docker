...
package: clean compile  # for internal use
	docker save ${IMG_NAME} -o  target/${ARTIFACT_NAME}-${VERSION}.tar
	git archive -o target/${ARTIFACT_NAME}-${VERSION}-src.tar.gz ${VERSION}
